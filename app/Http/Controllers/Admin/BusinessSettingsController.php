...
class BusinessSettingsController extends Controller
{
    ...
    {   
        ...
        DB::table('business_settings')->updateOrInsert(['key' => 'login_title_name'], [
            'value' => $request['login_title']
        ]);
        DB::table('business_settings')->updateOrInsert(['key' => 'login_tag_line'], [
            'value' => $request['login_tag_line']
        ]);
        ...
        $auth_curr_logo = BusinessSetting::where(['key' => 'auth_logo'])->first();

        if ($request->has('auth_logo')) {
            $image_name = Helpers::update('business/', $auth_curr_logo->value, 'png', $request->file('auth_logo'));
        } else {
            $image_name = $auth_curr_logo['value'] ?? '';
        }
        
        DB::table('business_settings')->updateOrInsert(['key' => 'auth_logo'], [
            'value' => $image_name
        ]);
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        } elseif ($name == 'monetbil') {
            DB::table('business_settings')->updateOrInsert(['key' => 'monetbil'], [
                'value' => json_encode([
                    'status' => $request['status'],
    ...
    {
        $tnc = BusinessSetting::withoutGlobalScope('translate')->where(['key' => 'terms_and_conditions'])->first();
        if ($tnc == false) {
    ...
    public function terms_and_conditions_update(Request $request)
    {
        BusinessSetting::where(['key' => 'terms_and_conditions'])->update([
            'value' => $request->tnc[array_search('en', $request->lang)]
        ]);

        $id = BusinessSetting::where(['key' => 'terms_and_conditions'])->first()->id;
        foreach($request->lang as $index=>$key)
        {
            if($request->tnc[$index] && $key != 'en')
            {
    ...
    {
        $data = BusinessSetting::withoutGlobalScope('translate')->where(['key' => 'privacy_policy'])->first();
        if ($data == false) {
            $data = [
                'key' => 'privacy_policy',
                'value' => '',
    ...
    public function privacy_policy_update(Request $request)
    {
        BusinessSetting::where(['key' => 'privacy_policy'])->update([
            'value' => $request->privacy_policy[array_search('en', $request->lang)],
        ]);


        $id = BusinessSetting::where(['key' => 'privacy_policy'])->first()->id;
        foreach($request->lang as $index=>$key)
        {
            if($request->privacy_policy[$index] && $key != 'en')
            {
                Translation::updateOrInsert(
                    ['translationable_type'  => 'App\Models\BusinessSetting',
                        'translationable_id'    => $id,
                        'locale'                => $key,
                        'key'                   => 'privacy_policy'],
                    ['value'                 => $request->privacy_policy[$index]]
                );
            }
        }
    ...
    {
        $data = BusinessSetting::withoutGlobalScope('translate')->where(['key' => 'about_us'])->first();
        if ($data == false) {
            $data = [
                'key' => 'about_us',
                'value' => '',
    ...
    public function about_us_update(Request $request)
    {
        BusinessSetting::where(['key' => 'about_us'])->update([
            'value' => $request->about_us[array_search('en', $request->lang)],
        ]);

        $id = BusinessSetting::where(['key' => 'about_us'])->first()->id;
        foreach($request->lang as $index=>$key)
        {
            if($request->about_us[$index] && $key != 'en')
            {
                Translation::updateOrInsert(
                    ['translationable_type'  => 'App\Models\BusinessSetting',
                        'translationable_id'    => $id,
