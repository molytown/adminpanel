...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        $restaurant = Restaurant::with('discount', 'zone')->selectRaw('*, IF(((select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and `restaurant_schedule`.`day` = '.$schedule_at->format('w').' and `restaurant_schedule`.`opening_time` < "'.$schedule_at->format('H:i:s').'" and `restaurant_schedule`.`closing_time` >"'.$schedule_at->format('H:i:s').'") > 0), true, false) as open')->where('id', $request->restaurant_id)->first();
        ...
        $order->zone_country = $restaurant->zone->zone_country;
        $order->zone_currency = $restaurant->zone->zone_currency;
