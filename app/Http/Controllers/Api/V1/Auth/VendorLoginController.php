...
class VendorLoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (auth('vendor')->attempt($data)) {
            ...
            return response()->json(['token' => $token,
                                    'currency' =>$vendor->restaurants[0]->zone->zone_currency,
                                    'country' =>$vendor->restaurants[0]->zone->zone_country,
                                    'zone_wise_topic'=> $vendor->restaurants[0]->zone->restaurant_wise_topic], 200);
        } else {
            $errors = [];
    ...
    public function register(Request $request)
    {
        ...
        $restaurant->email = $request->owner_email ? $request->owner_email : $request->email;
