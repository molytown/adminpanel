    ...
    public function login(Request $request)
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            return response()->json(['token' => $token,
                                    'currency' =>$delivery_man->zone->zone_currency,
                                    'country' =>$delivery_man->zone->zone_country,
                                    'zone_wise_topic'=>$delivery_man->type=='zone_wise'?$delivery_man->zone->deliveryman_wise_topic:'restaurant_dm_'.$delivery_man->restaurant_id], 200);
        } else {
            $errors = [];
