...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        foreach($zones as $country) {
            if($country->zone_country !=null){
                array_push($country_array, $country->zone_country);
           }
        }
    ...
    public function get_zone(Request $request)
    {
        ...
        $zones = Zone::contains('coordinates', $point)->latest()->get(['id', 'status', 'minimum_shipping_charge', 'per_km_shipping_charge', 'zone_country', 'zone_currency','cod','digital_payment']);
        if (count($zones) < 1) {
            return response()->json([
                'errors' => [
