                ...
                <form action="{{route('admin.business-settings.about-us')}}" method="post" id="tnc-form">
                    ...
                    @if ($language)
                        @foreach(json_decode($language) as $lang)
                            ...
                            <textarea class="ckeditor form-control" name="about_us[]" oninvalid="document.getElementById('en-link').click()">{!! $lang==$default_lang?$data['value']:($translate[$lang]['about_us']??'') !!}</textarea>
                            </div>
