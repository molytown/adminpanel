    ...
    <div class="content container-fluid">
        ...
        <div class="row digital_payment_methods gy-2 pb-3">
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body p-30px">
                        @php($config=\App\CentralLogics\Helpers::get_business_settings('monetbil'))
                        <form
                            action="{{env('APP_MODE')!='demo'?route('admin.business-settings.payment-method-update',['monetbil']):'javascript:'}}"
                            method="post">
                            @csrf
                            @if(isset($config))
                        <h5 class="d-flex flex-wrap justify-content-between">
                            <strong>{{translate('messages.monetbil')}}</strong>
                            <label class="switch--custom-label toggle-switch toggle-switch-sm d-inline-flex">
                                <span class="mr-2 switch--custom-label-text text-primary on text-uppercase">{{ translate('messages.on') }}</span>
                                <span class="mr-2 switch--custom-label-text off text-uppercase">{{ translate('messages.off') }}</span>
                                <input type="checkbox" class="toggle-switch-input" name="status" value="1" {{$config?($config['status']==1?'checked':''):''}}>
                        ...
                        <div class="payment--gateway-img">
                            <img src="{{asset('/public/assets/admin/img/payment/monetbil.png')}}" alt="public">
                        </div>
                            <div class="form-group mb-4">
                                <label class="{{Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3'}}">{{translate('messages.callback')}}</label>
                                <span class="btn btn-secondary btn-sm m-2"
                                    onclick="copyToClipboard('#id_monetbil')"><i class="tio-copy"></i> {{translate('messages.copy_callback')}}</span>

                                <p class="form-control" id="id_monetbil">{{ url('/') }}/monetbil-callback</p>
                            </div>
                            <div class="form-group mb-4">
                                <label class="{{Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3'}}">{{translate('messages.notification')}}</label>
                                <span class="btn btn-secondary btn-sm m-2"
                                    onclick="copyToClipboard('#id_monetbil_notification')"><i class="tio-copy"></i> {{translate('messages.copy_notification_url')}}</span>

                                <p class="form-control" id="id_monetbil_notification">{{ url('/') }}/monetbil-notification</p>
                            </div>
                            <div class="form-group mb-4">
                                <input class="form-control" type="text" placeholder="Service Key" name="service_key"
                                           value="{{env('APP_MODE')!='demo'?$config['service_key']:''}}">
                            </div>
                            <div class="btn--container justify-content-end">
                                <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}"
                                        onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}" class="btn h--37px btn--primary">{{translate('messages.save')}}</button>
