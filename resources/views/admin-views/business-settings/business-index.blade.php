        ...
        <div class="row gx-2 gx-lg-3">
            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="{{ route('admin.business-settings.update-setup') }}" method="post"
                    ...
                    <div class="card mb-3">
                        <div class="card-header">
                            <h4 class="card-title m-0 d-flex align-items-center"> <span class="card-header-icon mr-2"><i class="tio-neighborhood"></i></span> <span>{{translate('messages.login')}} {{translate('messages.setting')}}  {{translate('messages.info')}}</span></h4>
                        </div>
                        <div class="card-body pb-0">
                            <div class="row">
                                <div class="col-md-4">
                                    @php($login_title_name = \App\Models\BusinessSetting::where('key', 'login_title_name')->first())
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlInput1">
                                            {{ translate('messages.login_title_name') }}
                                        </label>
                                        <input type="text" name="login_title" value="{{ $login_title_name->value ?? '' }}" class="form-control"
                                            placeholder="{{ translate('messages.Ex :') }} Welcome to ABC Company" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    @php($login_tag_line = \App\Models\BusinessSetting::where('key', 'login_tag_line')->first())
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlInput1">{{ translate('messages.login_tag_line') }}</label>
                                        <input type="text" name="login_tag_line" value="{{ $login_tag_line->value ?? '' }}" class="form-control"
                                            placeholder="{{ translate('messages.Ex :') }} Manage your app & website easily" required>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-md-4">
                                    @php($auth_logo = \App\Models\BusinessSetting::where('key', 'auth_logo')->first())
                                    @php($auth_logo = $auth_logo->value ?? '')
                                    <div class="form-group mb-0">
                                        <center class="h-100px">
                                            <img class="initial-10"
                                                id="authViewer"
                                                onerror="this.src='{{ asset('public/assets/admin/img/160x160/img2.jpg') }}'"
                                                src="{{ asset('storage/app/public/business/' . $auth_logo) }}" alt="logo image" />
                                        </center>
                                        <label class="input-label mt-2">{{ translate('messages.login') }} {{ translate('messages.logo') }} <small class="text-danger">* ( {{ translate('messages.ratio') }} 300x100 )</small></label>
                                        <div class="custom-file mb-3">
                                            <input type="file" name="auth_logo" id="authImageFile" class="custom-file-input"
                                                accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                            <label class="custom-file-label" for="authImageFile">{{ translate('messages.choose') }}
                                                {{ translate('messages.file') }}</label>
                                        </div>
                                    </div>
                                </div>
...
@push('script_2')
    <script>
        ...
        $("#authImageFile").change(function() {
            readURL(this, 'authViewer');
        });
    </script>
    <script
